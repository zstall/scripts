...
local sql=$(cat <<EOF
\COPY (select (date_updated - date_created) as duration, execution_label, date_created, date_updated, status from workflow_execution where date_created between '${start}' and '${end}' order by duration desc) to '~/${currentDate}-All-workflows.csv' with csv header;
EOF
)
   echo "$sql"
}
...
local attachmentOne="~/${currentDate}-Avg-workflows.csv"
local attachmentTwo="~/${currentDate}-All-workflows.csv"
